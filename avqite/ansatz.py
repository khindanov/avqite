...
import numpy, pickle, warnings, itertools
...
class ansatz:
    ...
    def init_ansatz(self):
        ...
        # ansatz_simp_filename = (self._model_dir + f"data/{self._filename}/ansatz_simp_tetras{self._tetras}.pkle")
        # if os.path.isfile(ansatz_simp_filename):
        #     self.pload_ansatz_simp(ansatz_simp_filename)
        # ansatz_inp_filename = (self._model_dir + f"data/{self._filename}/ansatz_inp_o"+self._config["optimize"]+"_s"+
    ...
    def get_scores(self, inds_chk):
        ...
        self._comm.Allreduce([self._mmat_pp_buf, MPI.DOUBLE],[mmat_pp_buf_allreduce, MPI.DOUBLE])
        ...
        for _ in range(self._max_add):
            ...
            for i, idx in enumerate(isort):
                ...
                if idx in ichoose or \
                    numpy.any([self._pool[idx][iq] != "I" for iq in qsupport]):
                    continue
            ...
            if self._mrank == 0:
                if self._nq <= 12:
                    print(" add op:", [self._ansatz[i] for i in range(self._layer_range[-1][0], self._layer_range[-1][1])])
                print(" add op:", [int(self._ansatz_pids[i]) for i in range(self._layer_range[-1][0], self._layer_range[-1][1])])
    ...
    def psave_ansatz_simp(self):
        # pass
        # for continuous run
        with open(self._model_dir + f"data/{self._filename}notetras/ansatz_simp_o"+self._config["optimize"]+"_s"+
                      self._config["simplify_sequence"]+"_b"+str(self._config["backend"])+f"_iter{self._iter}_tetras{self._tetras}.pkle", "wb") as f:
    ...
    def psave_ansatz_inp(self):
        ...
        with open(self._model_dir + f"data/{self._filename}notetras/ansatz_inp_o"+self._config["optimize"]+"_s"+
                      self._config["simplify_sequence"]+"_b"+str(self._config["backend"])+f"_iter{self._iter}_tetras{self._tetras}.pkle", "wb") as f:
            data = [self._ansatz,
                    self._params,
                    ]
    ...
    def psave_tn_w_c(self, width, cost, tn_type, freq = 10):
        ...
        if self._iter % freq == 0:
            with open(self._model_dir + f"data-width-cost/{self._filename}notetras/data_wc_calc_{tn_type}_"+
    ...
    def psave_circ(self, circ, tn_type, width, cost):
        ...
        if self._mrank in ranks:
            with open(self._model_dir + f"data-circuits/{self._filename}notetras/data_circ_rank{self._mrank}_tetras{self._tetras}.pkle", "wb") as f:
                data=[width,
                    cost,
                    circ.gates, 
                    tn_type, 
...
class ansatzSinglePool(ansatz):
    ...
    def set_pool_gates(self):
        ...
        # pauli string gate
        self._poolpgates = [
                quimbify.pauli_string_to_quimb_gates(p)
                for p in self._pool
                ]
    ...
    def set_base_circs(self):
        ...
        for ind in self._ansatz_pids[ncircs:]:
            circ = circ.copy()
            circ.apply_gates(self.get_poolrgate(ind),
                    contract=False,
                    )
    ...
    def get_mcirc(self, i, j, gate_i=None, gate_j=None):
        ...
        circ.apply_gates(gate_j,
                contract=False,
                )

        for k in range(j+1, i):
            circ.apply_gates(self.get_poolrgate(self._ansatz_pids[k]),
                    contract=False,
                    )
        circ.apply_gates(gate_i, contract=False,)
    ...
    def calc_mmat(self):
        ...
        mmat[self._minds[0], self._minds[1]] = \
                mmat[self._minds[1], self._minds[0]] = mvals
        mmat_width[self._minds[0], self._minds[1]] = \
                mmat_width[self._minds[1], self._minds[0]] = mvals_width
        mmat_cost[self._minds[0], self._minds[1]] = \
                mmat_cost[self._minds[1], self._minds[0]] = mvals_cost
        ...
        self._comm.Reduce([mmat_width, MPI.DOUBLE],[mmat_width_reduce, MPI.DOUBLE], root=0)

        mmat_cost_reduce = numpy.empty(numpy.shape(mmat_cost), dtype=float)
        self._comm.Reduce([mmat_cost, MPI.DOUBLE],[mmat_cost_reduce, MPI.DOUBLE], root=0)

        if self._mrank == 0:
            nzeros = numpy.count_nonzero(abs(mmat) < 1e-12)
            print(f" zero fraction of mmat: {nzeros}/{mmat.size} or {nzeros//2}/{(mmat.size-len(mmat))//2}")
        ...
        for i, x in zip(itertools.count(), self._params):
            ...
            for j, dtheta in enumerate([numpy.pi/2, -numpy.pi/2]):
                ...
                for k, op in enumerate(self._pstrings_h):
                    if icount%self._msize == self._mrank:
                        if self._model._localh:
                            exp_val = circ.local_expectation(op[0], op[1],
                                    optimize=self._config["optimize"],
                                    simplify_sequence=self._config["simplify_sequence"],
                                    )
                        else:
                            exp_val = self.get_expectation_via_amplitude(op, params=params)[0]
    ...
    def calc_v_pd(self):
        ...
        chunk_sizes = [(np*self._hsize) // self._msize + (1 if i < rem else 0)
        ...
        for param in params_in_chunk:
            ...
            circ.apply_gates(self._poolpgates[self._ansatz_pids[param]],
                contract=False,
                )
            for k in self._ansatz_pids[param+1:]:
                circ.apply_gates(self.get_poolrgate(k),
                        contract=False,
                        )
        ...
        for _,ind in enumerate(range(start, end)):
            ...
            circ.apply_gates(op_gates, contract=False,)
        ...
        self._comm.Reduce([vals_width, MPI.DOUBLE],[vals_width_reduce, MPI.DOUBLE], root=0)

        vals_cost_reduce = numpy.empty(numpy.shape(vals_cost), dtype=float)
        self._comm.Reduce([vals_cost, MPI.DOUBLE],[vals_cost_reduce, MPI.DOUBLE], root=0)
        ...
        for i, op in enumerate(self._pstrings_h):
            if i%self._msize == self._mrank:
                if self._model._localh:
                    ...
                    exp_val = self.get_expectation_via_amplitude(op,
    ...
    def calc_e2(self):
        ...
        vals_hh = [self.get_expectation_via_amplitude(op)[0].real
    ...
    def quimbify_pstrings(self):
        if self._model._localh:
            ...
            self._pstrings_h = [
                    quimbify.pauli_string_to_quimb_op(p) for p in \
                            self._pstrings_h
                    ]
        else:
            # to quimb gates
            self._pstrings_h = [
                    quimbify.pauli_string_to_quimb_gates(p) for p in \
                            self._pstrings_h
                    ]
    ...
    def set_pstrings_hh(self):
        ...
        self.chunk_sizes_hh = [self._hhsize//self._msize + (1 if i < rem else 0)
                for i in range(self._msize)]
        self.shifts_hh = [sum(self.chunk_sizes_hh[:i]) for i in range(self._msize)]
        if self._mrank == 0:
            chunks = [self._pstrings_hh[self.shifts_hh[i]:self.shifts_hh[i]+self.chunk_sizes_hh[i]]
                    for i in range(self._msize)]
    ...
    def set_poolh(self):
        ...
        sorted_inds = numpy.argsort(pstrings)
        pstrings = pstrings[sorted_inds]
        poolh[1] = [sorted_inds[numpy.searchsorted(pstrings, oplabels)]
                for oplabels in poolh[1]]
        self._poolh = poolh

        # split and distribute pstrings
        rem = self._phsize%self._msize
        self.chunk_sizes_ph = [self._phsize//self._msize + (1 if i < rem else 0)
                for i in range(self._msize)]
        self.shifts_ph = [sum(self.chunk_sizes_ph[:i]) for i in range(self._msize)]

        if self._mrank == 0:
            chunks = [self._pstrings_ph[self.shifts_ph[i]:self.shifts_ph[i]+self.chunk_sizes_ph[i]]
                    for i in range(self._msize)]
        else:
    ...
    def get_mv(self,
            ...
            for shift, inds_cur in zip(
                ...
                for j, ind_j in enumerate(inds_cur):
                    ...
                    if self._mmat_pp[ij] > 0.5:   # havenot been evaluated
                        if ((parallel and icount%self._msize == self._mrank)
                                or not parallel):
                            circ = self.get_mcirc(
                                    self._ntheta_old,
                                    self._ntheta_old-1,
                            ...
                            if abs(res) < 1e-16:   # for marking purpose
                                res = 1e-16
                            self._mmat_pp_buf[ij] = \
                                    mmat[ntheta+i, shift+j] = \
                                    mmat[shift+j, ntheta+i] = res
                        icount += 1
                    else:
                        if ((parallel and icount%self._msize == self._mrank)
                                or not parallel):
                            mmat[ntheta+i, shift+j] = \
                                    mmat[shift+j, ntheta+i] = self._mmat_pp[ij]
        ...
        for i, ind_i in enumerate(inds):
            mmat[ntheta+i, :self._ntheta_old] = \
                    mmat[:self._ntheta_old, ntheta+i] = \
                    self._mmat_pt[ind_i, :]
